        ...
        public class SMPPClient
        {
            ...
            public void Send(byte[] data, int n)
            {
                try
                {
                    ...
                    var result = clientSocket.BeginSend(data, 0, n, 0, new AsyncCallback(sendCallback), clientSocket);
            ...
            public int SendSms(String to, String text)
            {
                return SendSms(to, splitLongText, text, askDeliveryReceipt);
            }//SendSms
            public int SendSms(String to, String text, byte askDeliveryReceipt)
            {
                return SendSms(to, splitLongText, text, askDeliveryReceipt);
            }//SendSms
            public int SendSms(String to, bool splitLongText, String text)
            {
                return SendSms(to, splitLongText, text, askDeliveryReceipt);
            }//SendSms
            public int SendSms(String to, bool splitLongText, String text, byte askDeliveryReceipt)
            {
                return SendSms(to, splitLongText, text, askDeliveryReceipt, 0, Tools.GetDataCoding(text));
            }

            public int SendSms(String to, bool splitLongText, String text, byte askDeliveryReceipt, byte esmClass, byte dataCoding)
            {
                int messageId = -1;
                ...
                sourceAddress = Tools.GetString(smscArray.currentSMSC.SMSFrom, 20, "");
            ...
            public int SendFlashSms(String to, String text)
            {
                return SendFlashSms(to, text, askDeliveryReceipt);
            }//SendFlashSms
            public int SendFlashSms(String to, String text, byte askDeliveryReceipt)
            {
                return SendSms(to, false, text.Substring(0, text.Length > 160 ? 160 : text.Length), askDeliveryReceipt, 0, 0x10);
            }//SendFlashSms
            public int SendMedia(String to, byte[] media)
            {
                return SendMedia(to, media, askDeliveryReceipt);
            }//SendMedia
            public int SendMedia(String to, byte[] media, byte askDeliveryReceipt)
            {
                return SendData(to, media, 0x40, 0xF5, askDeliveryReceipt);
            }//SendMedia
            public int SendData(String to, byte[] data)
            {
                return SendData(to, data, askDeliveryReceipt);
            }//SendData
            public int SendData(String to, byte[] data, byte askDeliveryReceipt)
            {
                return SendData(to, data, 0, 0, 0);
            }//SendData
            public int SendData(String to, byte[] data, byte esmClass, byte dataCoding, byte askDeliveryReceipt)
            {
                int messageId;
                byte sourceAddressTon;
                byte sourceAddressNpi;
                ...
                destinationAddress = Tools.GetString(to, 20, "");//from -> to
                destinationAddressTon = getAddrTon(destinationAddress);
                destinationAddressNpi = getAddrNpi(destinationAddress);
            ...
            private void connectToSMSC()
            {
                try
                {
                    ...
                    clientSocket.SendTimeout = mTimeout;
                    clientSocket.ReceiveTimeout = mTimeout;
                    ...
                    var result = clientSocket.BeginConnect(remoteEP, new AsyncCallback(connectCallback), clientSocket);
            ...
            private void receive()
            {
                try
                {
                    ...
                    var result = clientSocket.BeginReceive(state.buffer, 0, StateObject.BufferSize, 0,
            ...
            private void logMessage(int logLevel, string pMessage)
            {
                try
                {
                    if(this.LogLevel > 0) //if ((this.LogLevel & logLevel) > 0)
                    {
